...
## 最后总结
在Web系统中，技术模型都是可以互相借鉴的，大部分语言都是图灵完备的，这意味着语言之间具有可替代性。语言会变，框架会变，但是解决问题的思路是不变的。
...
#### 技术栈列表
1. python3.8
2. WSGI，所有Web框架都会实现它，但有些框架直接使用socket
3. Git:工作必备
4. 编码规范
5. Bootstrap/前端技能：可以提高竞争优势。
6. Django：一站式Web开发
7. xadmin：没啥用已停更。
8. django-rest-framework：大潮流。
9. 调试和优化
10. cache，理解运行原理，为什么要使用它，redis
11. Gunicorn：只要事先了WSGI协议，就可以使用Gunicorn
12. 分发和部署：按照标准来就好
13. 自动化部署：前提是将人为操作的步骤变为清晰可定义的机器能执行的操作。原则是一次开发，无数次轻松使用。
14. Nginx：至少需要能够独立完成Nginx的部署
15. 监控和统计：根据情况实际选择。
16. 压力测试
...
1. 部署到vps上
2. 优化系统结构和代码
3. 配置CI：持续集成(github上配置CI)
4. 文章爬取
5. 缓存算法优化
6. 热门文章逻辑--了解下热度算法优化逻辑。
7. 内容推荐
8. 搜索
...
当函数不带括号时，它是函数本身，可以被传递，被赋值给别的变量；当函数带括号时，这个函数就会被执行。
